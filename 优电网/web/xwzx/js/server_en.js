...
$.ajax({
	...
	cache: false,
	type: 'GET',
	url: 'http://api.ncfun.top:2021/powergird/web/news/list/xwzx_en',
	success: function (result) {
...
function updata_News(result, nowPage) {
		...
		if (newslist[i].type == "News") {
			if (nowNews >= needMinNews && nowNews <= needMaxNews) {
				...
				if (newslist[i].content != null) {
					dds = newslist[i].content.substring(0, 360) + "……";
	...
	if (nowPage != 1) {
		pageDiv.innerHTML += "<a href='index_en.html?page=" + (nowPage - 1) + "'><div class='row_left_page_item'><h1><</h1></div></a>";
	}
	for (var i = 0; i < totalPage; i++) {
		...
		pageDiv.innerHTML += "<a href='index_en.html?page=" + (i - 1 + 2) + "'><div class='" + itmePageDivClassName + "'><h1>" + (i + 1) + "</h1></div></a>"
	}
	if (nowPage != totalPage) {
		pageDiv.innerHTML += "<a href='index_en.html?page=" + (Number(nowPage) + 1) + "'><div class='row_left_page_item'><h1>></h1></div></a>"
...
$.ajax({
	...
	cache: false,
	type: 'GET',
	url: 'http://api.ncfun.top:2021/powergird/web/news/list/hits/xwzx_en',
	success: function (result) {
...
function updata_HotOrLastNews(result, id) {
	...
	for (var i = 0; i < 8; i++) {
		...
		newslist[i].title = newslist[i].title.substring(0, 32) + "……";

		newsDiv.innerHTML = "<a target='_blank' href='" + newslist[i].relativeUrl + "'><h1>" + (Number(i + 1)) + "</h1><p onmouseover='right_info_over()' onmouseleave='right_info_out()'>" + newslist[i].title + "</p></a>"
