...
$.ajax({
    ...
    cache:false,
    type:'GET',
    url:'http://api.ncfun.top:2021/powergird/web/news/list/sy',
    success:function(result){
        var obj = JSON.parse(JSON.stringify(result));
        var newsList = eval(obj.news);
        for(var i = 0; i < newsList.length; i++){
            //新闻资讯
            if(newsList[i].type == "新闻资讯"){
                if(newsList[i].sequence == 1){
                    ...
                    div_content.innerHTML = "<a href = "+ newsList[i].relativeUrl +"><h1 style='font-size:34px;'>"+newsList[i].createTime.substring(5,11) + newsList[i].createTime.substring(0,4) +"</h1><h3>"+ newsList[i].title +"</h3><p>"+ newsList[i].subtitle +"</p> </a>";
            ...
            if(newsList[i].type == "热点专题"){
                ...
                hot_content.innerHTML = "<a href = " + newsList[i].relativeUrl + " ><h3> HOT&nbsp TOPICS</h3><h1>热点专题</h1>"+
            ...
            if(newsList[i].type == "政策方针"){
                ...
                blue_block_url.href = newsList[i].relativeUrl;
            }
        }
    }
});