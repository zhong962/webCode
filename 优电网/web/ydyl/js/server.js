		...
		for(var i = 0; i < newslist.length;i++){
			...
			if(newslist[i].type == "发展状况"){
				if(newslist[i].sequence == 1){
					...
					statusContent.innerHTML = "<h4><br>"+ newslist[i].subtitle +"</h4><h1><b>"+ newslist[i].title +"</b></h1><p><br>"+newslist[i].content +"</p><a href="+ newslist[i].relativeUrl +" target='_blank'><h5>MORE</h5></a>";
				}else{
					var div = document.getElementById("status2_div");
					var divNews = document.createElement("div");
					var images = eval(newslist[i].images);
					divNews.innerHTML = "<figure class=imghvr-shutter-out-horiz><img src="+ images[0].image +"><figcaption class=status-fig-text style='margin-top:46%'>"+ newslist[i].title +"</figcaption><a href="+ newslist[i].relativeUrl +" ></a></figure>";
